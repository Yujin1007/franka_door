...
class door_template:
    ...
    def reset(self, planning_mode):
        ...
        self.start_time = self.data.time + 100
        self.controller.initialize(planning_mode, self.latch_info, self.door_info,
                                   [self.goal_latch_angle, self.goal_door_angle])
    ...
    def step(self, action_rotation, action_force):
        ...
        duration = 0
        action_force = self.recalc_action(action_force)
    ...
    def _observation(self):
        ...
        self.obs_dq = (dq_unscaled - self.qdot_range[:, 0]) / (self.qdot_range[:, 1] - self.qdot_range[:, 0]) * (
                    1 - (-1)) - 1
    ...
    def recalc_action(self, action_force):
...
class door_env2(door_template):
    def _reward(self, action_rotation, action_force):
        ...
        # print("d door angle:",(self.door_angle - self.door_angle_pre), "omega:", self.obs_omega[0] )
        q_max = max(abs(self.obs_q))
        reward_door = 0.0
        if q_max > 0.9:
            if action_force < 0:
            ...
            if 0.1 <= abs(self.obs_omega[0]) <= 0.15:
                reward_force += 1
                # reward_door = 1
            elif abs(self.obs_omega[0]) < 0.15:
                door_open_angle = (self.door_angle - self.door_angle_pre) * 5e2
                reward_door = np.clip(door_open_angle, door_open_angle, 1)
        ...
        return reward_rotation, reward_force+reward_door
